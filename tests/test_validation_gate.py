...
class ValidationGateTests(unittest.TestCase):
    ...
    def test_only_policy_advances_state(self) -> None:
        ...
        matches = list(
            re.finditer(update_pattern, content, re.IGNORECASE | re.MULTILINE)
        )
    ...
    def test_stripe_idempotency_unique(self) -> None:
        ...
        with open_db(self._path) as conn:
            ...
            cur = conn.execute(
                "SELECT sql FROM sqlite_master WHERE type='table' AND name='payment_confirmations'"
            )
    ...
    def test_provenance_required(self) -> None:
        ...
        with open_db(self._path) as conn:
            ...
            cur = conn.execute(
                "SELECT sql FROM sqlite_master WHERE type='table' AND name='evidence_ledger'"
            )
            schema = cur.fetchone()["sql"]
            ...
            storage_path = (
                Path(__file__).parent.parent / "src" / "xchange" / "storage.py"
            )
            content = storage_path.read_text()
            # Find all append_evidence calls
            append_calls = re.findall(
                r'append_evidence\([^)]+provenance=["\']([^"\']+)', content
            )
            for provenance in append_calls:
                self.assertTrue(
                    len(provenance) > 0,
                    "Found empty provenance string in append_evidence call",
                )
    ...
    def test_support_path_on_missing_metadata(self) -> None:
        ...
        with open_db(self._path) as conn:
            ...
            cur = conn.execute(
                "SELECT kind FROM support_signals ORDER BY id DESC LIMIT 1"
            )
            row = cur.fetchone()
