...
def _parser() -> argparse.ArgumentParser:
    ...
    p = argparse.ArgumentParser(
        description="Build and optionally submit a Unified Session Evidence Bundle."
    )
    ...
    p.add_argument(
        "--grid-root",
        type=Path,
        help="Accepted for compatibility; v1 uses --grid-lumos-path or Seeds snapshots.",
    )
    ...
    p.add_argument("-o", "--output", default="-", help="Write bundle JSON path, or '-' stdout.")
    return p
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if not args.submit:
            out_json = json.dumps(bundle, ensure_ascii=False, indent=2, sort_keys=True)
            if args.output == "-":
                print(out_json)
            else:
